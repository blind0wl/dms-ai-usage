...
assert_eq "$(val "$OUT5" ACCOUNT_PRIMARY_UTIL)" "default:1,work:80" "ACCOUNT_PRIMARY_UTIL carries each Account's own rolling.percent"
...
assert_eq "$(val "$OUT7" ACCOUNT_CREDS_STATUS)" "default:ok,work:missing" "a rejected secondary key is reported as missing, not hidden behind the healthy default"
