...
2. Queries opencode's undocumented Go usage endpoint for the account-level 5-hour (`rolling`), weekly (`weekly`) and monthly (`monthly`) window utilization and reset times. The monthly entry is the tertiary Window: it renders as a third row in the Windows card with no pacing tick, competes for Tightest Window while the pill ring stays on primary, and is omitted entirely when the response does not carry it. Window lengths are declared as 5 hours, 7 days and about 30 days, since the endpoint reports reset times rather than durations. There is no plan name and no spend in the response, so the tab carries only the Source name and the Window card
