...
**Follow-up fix (found after ticket closed, during [Build Claude source](04-build-claude-source.md)):** the rename left `ClaudeCodeUsageWidget.qml`'s `scriptPath` hardcoded to `PluginService.pluginDirectory + "/claudeCodeUsage/get-claude-usage"` — the old plugin id's folder name, which no longer exists (the script lives at the `aiUsage` folder's root now). The widget silently found nothing to run and stayed at zero, which is exactly the "not showing any usage" symptom the user hit live. Fixed to `PluginService.pluginDirectory + "/" + root.pluginId + "/get-claude-usage"`, using the `pluginId` the host already injects from `plugin.json`, so it can't drift out of sync on a future rename. Verified live: `dms restart`, `~/.claude/usage-cache.json` timestamp updated immediately confirming the script actually ran and hit the API.
